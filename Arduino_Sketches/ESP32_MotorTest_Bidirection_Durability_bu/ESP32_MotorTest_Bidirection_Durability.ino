...
void SensorTask(void *pvParameters) {
    for (;;) {
        ...
        if (time_now - lastSPortReq > 10) {
            lastSPortReq = time_now;
            serial1SendIdRequest(1);
        }
...
void loop()
{
    ...
    if (localT - lastPrint >= 200) {
        ...
        if (low_power_flag) {
            ...
            printDShotInfo(myMotor, Serial);
            // dshot_result_t result = myMotor.getTelemetry();
            // dshot_telemetry_data_t telem_result = result.telemetry_data;
            // Serial.printf("%.2f,%d,%.2f,%.2f\n", telem_result.voltage, motorDShotVal, telem_result.rpm, force_reading);
